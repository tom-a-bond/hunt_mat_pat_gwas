...
writeLines('Summary of residuals regresison model:')
print(summary(model))
...
write.table(resid[c('V1', 'V2', 'resid')], file = paste0(pheno_file, '.resid'), ro = FALSE, co = FALSE, qu = FALSE)
...
if(sample == '4m'){
  ...
  index_parent_id = 'id_m'
  other_parent_id = 'id_f'
}
if(sample == '4f'){
  other_parent_geno = read.table(mother_geno_file, header = TRUE)
  index_parent_id = 'id_f'
  other_parent_id = 'id_m'
}
...
write.table(geno_m2, paste0(out_path, 'qcovars_', snp, '.qcovar'), ro = FALSE, co = FALSE, qu = FALSE)
