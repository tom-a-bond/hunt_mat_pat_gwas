...
if(is.na(cm) | is.na(cp) | is.na(mp)){
  stop('one or more LDSC intercepts appear to be missing')
}

# apply donuts
donuts <- donuts(ss.own = off, ss2 = mat, ss3 = pat, mode = 1, 
                 l12 = cm, l13 = cp, l23 = mp, alpha = alpha,
                 OutDir = NULL)  
...
donuts <- donuts[ , c("rsid", "SNP", "CHR", "BP", "A1", "A2",
                      ...
                      "beta.ss3", "se.ss3", "p.ss3", "n.ss3",
                      "beta.own", "se.own", "p.own", "n.own",
                      "beta.ind.mat", "se.ind.mat", "p.ind.mat", "n.ind.mat",
                      "beta.ind.pat", "se.ind.pat", "p.ind.pat", "n.ind.pat",
...
names(donuts) <- c("rsid", "chr_pos_a1_a2", "chr", "pos", "a1", "a2",
                   ...
                   "beta_pat_donuts", "se_pat_donuts", "p_pat_donuts", "n_pat_donuts",
...
m <- merge(mat_m, pat_m, by.x = 'SNP_mat', by.y = 'SNP_pat', all.x = TRUE)
m <- merge(m, off_m, by.x = 'SNP_mat', by.y = 'SNP_off', all.x = TRUE)
m <- m[which(!is.na(m$Freq1_mat) & !is.na(m$Freq1_pat) & !is.na(m$Freq1_off)), ]
# there should not be any discrepant alleles:
if(nrow(m[which(m$A1_mat != m$A1_pat | m$A1_mat != m$A1_off | m$A1_pat != m$A1_off), ]) > 0){
  stop('alleles appear to be inconsistent between maternal, paternal and/or offspring gwas')
}
names(m) <- c('SNP',
              ...
              paste0(c('freq_a1', 'freq_a1_se', 'het_isq', 'het_chisq', 'het_df', 'het_p', 'n', 'a1', 'a2'), '_pat'),
              paste0(c('freq_a1', 'freq_a1_se', 'het_isq', 'het_chisq', 'het_df', 'het_p', 'n', 'a1', 'a2'), '_off'))
donuts_backup=donuts
donuts$n_mat <- NULL
donuts$n_pat <- NULL
donuts$n_off <- NULL
...
donuts <- donuts[ , c("rsid", "chr_pos_a1_a2", "chr", "pos", "a1", "a2", 'freq_a1', 'freq_a1_se',
                      ...
                      "beta_pat", "se_pat", "p_pat", paste0(c('het_chisq', 'het_df', 'het_p', 'n'), '_pat'),
